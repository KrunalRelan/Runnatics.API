...
namespace Runnatics.Services
{
                                ...
                                IConfiguration configuration) : ServiceBase<IUnitOfWork<RaceSyncDbContext>>(repository), IAuthenticationService
    {
        ...
        public Task<InvitationResponse?> InviteUserAsync(InviteUserRequest request, int tenantId, int invitedBy)
        {
        ...
        public async Task<AuthenticationResponse?> RegisterOrganizationAsync(RegisterOrganizationRequest request)
        {
            try
            {
                ...
                var existingOrganization = organizationRepo
                                    .GetQuery(o => o.Name == request.OrganizationName
                ...
                var existingUser = userRepo
                            .GetQuery(u => u.Email == request.SuperAdminEmail
                                     && u.AuditProperties.IsActive
                                     && !u.AuditProperties.IsDeleted)
                            .FirstOrDefault();
                ...
                await _repository.ExecuteInTransactionAsync(async () =>
                {
                    ...
                    addedOrganization = await organizationRepo.AddAsync(organization);
                    await _repository.SaveChangesAsync();

                    var superAdminUser = new User
                    {
                        TenantId = addedOrganization.Id,
                    ...
                    addeduser = await userRepo.AddAsync(superAdminUser);
                });
                ...
                response.ExpiresAt = DateTime.UtcNow.AddMinutes(30);
